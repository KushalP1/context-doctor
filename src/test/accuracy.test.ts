...
function assistantLine(text: string, input: number): string {
...
test("usage samples are recorded per assistant turn, positioned in the message array", async () => {
  ...
  writeFileSync(
    path,
    [userLine("hello"), assistantLine("hi", 1000), userLine("more"), assistantLine("ok", 3000)].join("\n") + "\n"
  );

  const parsed = parseSessionFile(path);
  assert.deepEqual(
    parsed.usageSamples,
    [{ index: 1, input: 1000 }, { index: 3, input: 3000 }],
    "each sample sits at the index of the assistant message that reported it"
  );
});
