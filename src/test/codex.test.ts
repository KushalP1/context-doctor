...
const line = (type: string, payload: unknown, ts = "2026-09-19T10:00:00.000Z") => JSON.stringify({ timestamp: ts, type, payload });
...
test("a Codex rollout parses into messages, tool calls, model and measured usage", () => {
  ...
  const conv = JSON.parse(p.conversationJson) as { messages: Array<{ role: string; content: unknown }> };
  const call = conv.messages[1].content as Array<{ type: string; id: string; name: string; input: unknown }>;
  assert.equal(call[0].type, "tool_use");
  assert.equal(call[0].id, "call_1", "custom_tool_call maps to tool_use keyed by call_id");
  const patch = conv.messages[3].content as Array<{ input: unknown }>;
...
test("the hook answers Codex's payload with Claude-shaped output Codex accepts", async () => {
  ...
  const out = await new Promise<string>((resolve, reject) => {
    const child = execFile(
      process.execPath,
      [cliPath, "hook"],
      { env: { ...process.env, CONTEXT_DOCTOR_HOOK_STATE: join(dirname(transcript), "state.json"), CONTEXT_DOCTOR_WARN_TOKENS: "5000" } },
  ...
  const r = JSON.parse(out) as { hookSpecificOutput: { hookEventName: string; additionalContext: string } };
...
test("install wires Codex when ~/.codex exists and uninstall removes only ours", async () => {
  ...
  const hooks = JSON.parse(readFileSync(join(home, ".codex", "hooks.json"), "utf8")).hooks.UserPromptSubmit as Array<{ hooks: Array<{ command: string }> }>;
  ...
  const left = JSON.parse(readFileSync(join(home, ".codex", "hooks.json"), "utf8")).hooks.UserPromptSubmit as Array<{ hooks: Array<{ command: string }> }>;
