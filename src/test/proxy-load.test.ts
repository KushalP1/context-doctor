...
test("60 concurrent streaming requests are accounted exactly", async () => {
  ...
  try {
    ...
    const stats = (await fetch(`http://127.0.0.1:${port}/stats`).then((r) => r.json())) as {
      requests: number; optimizedRequests: number; upstreamInputTokens: number; upstreamOutputTokens: number; estUsdSaved: number;
    };
