...
async function lifetimeSaved(): Promise<number> {
...
test("lifetime totals survive ledger rotation exactly", async () => {
  ...
  try {
    ...
    if (previous === undefined) delete process.env.CONTEXT_DOCTOR_HOOK_STATE;
    else process.env.CONTEXT_DOCTOR_HOOK_STATE = previous;
  }
});
