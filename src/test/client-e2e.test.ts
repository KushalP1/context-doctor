...
async function exercise(client: Client): Promise<void> {
  ...
  const summary = (optimized.content as Array<{ text?: string }>)[0]?.text ?? "";
  ...
  assert.match((tips.content as Array<{ text?: string }>)[0]?.text ?? "", /cache_control/);
}
  ...
  try {
    ...
    for (;;) {
      try {
        ...
        if (Date.now() > deadline) throw new Error("http server never came up");
