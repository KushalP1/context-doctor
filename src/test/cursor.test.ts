...
let DatabaseSync: (new (p: string, o?: unknown) => any) | undefined;
try {
...
function buildFixture(): string {
  ...
  db.prepare("INSERT INTO cursorDiskKV VALUES (?, ?)").run(
    `bubbleId:${modernId}:b1`,
    JSON.stringify({ type: 1, text: "how do I fix the migration?" })
  );
  db.prepare("INSERT INTO cursorDiskKV VALUES (?, ?)").run(
    `bubbleId:${modernId}:b2`,
    JSON.stringify({ type: 2, text: "Let me look at the schema." })
  );
  db.prepare("INSERT INTO cursorDiskKV VALUES (?, ?)").run(
    `bubbleId:${modernId}:b3`,
    JSON.stringify({
      type: 2,
      text: "",
...
test("reads the modern shape: bubble rows, roles, and tool calls", { skip }, () => {
  ...
  const conv = JSON.parse(parsed.conversationJson) as { messages: Array<{ role: string; content: any }> };
...
test("reads the legacy shape: inline conversation text", { skip }, () => {
  ...
  const conv = JSON.parse(parsed.conversationJson) as { messages: Array<{ role: string; content: string }> };
