...
const post = (path: string) => fetch(base + path, { method: "POST", headers: { "content-type": "application/json", authorization: "Bearer k" }, body });
...
test("proxy token: good prefix is served and stripped, bad or missing prefix gets 401 without an upstream call", async () => {
  ...
  const json = (await ok.json()) as { path: string };
