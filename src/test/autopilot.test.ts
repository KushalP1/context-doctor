...
const big = (n: number) => "const value = compute(input, options); // step\n".repeat(n); // ~48 chars/line, code
function conversation(results: number, opts: { tool?: string; lines?: number } = {}) {
...
test("cold cache: old re-runnable results are cleared in one batch, the last 3 stay", () => {
  ...
  const contents = body.messages.filter((m: any) => m.role === "user" && Array.isArray(m.content)).map((m: any) => m.content[0].content);
  assert.ok(contents.slice(0, 9).every((x: string) => x.startsWith("[context-doctor: old tool output cleared")));
  assert.ok(contents.slice(9).every((x: string) => x.startsWith("const value")), "last 3 untouched");
...
test("a malformed request is forwarded unchanged, never an error", () => {
  const r = new AutoClearer().apply({ messages: "nope" } as any, 0);
  assert.equal(r.changed, false);
});
...
const proxy = startProxy({ port: 0, anthropicUpstream: `http://127.0.0.1:${(upstream.address() as AddressInfo).port}`, autopilot: true, autopilotPauseFile: pauseFile });
...
const send = (body: unknown) => fetch(`${base}/v1/messages`, { method: "POST", headers: { "content-type": "application/json", "x-api-key": "k" }, body: JSON.stringify(body) }).then((r) => r.json());
...
test("proxy autopilot: first (cold) request goes out lighter; stats and /health say so", async () => {
  ...
  const stats = await fetch(`${base}/stats`).then((r) => r.json()) as any;
