...
test("the conversation's own model field picks the ratios in profile and optimize", () => {
  const body = (model: string) => JSON.stringify({ model, messages: [{ role: "user", content: prose.repeat(100) }] });
...
test("measureTokenizer recovers exact ratios from usage in a transcript", () => {
  ...
  const usage = (prompt: number, output: number) => ({ input_tokens: 2, cache_read_input_tokens: prompt - 2, cache_creation_input_tokens: 0, output_tokens: output });
  const lines = [
...
test("calibration learned against the old heuristic is ignored, then restarted", async () => {
  ...
  try {
    ...
    if (saved.state === undefined) delete process.env.CONTEXT_DOCTOR_HOOK_STATE; else process.env.CONTEXT_DOCTOR_HOOK_STATE = saved.state;
    if (saved.off !== undefined) process.env.CONTEXT_DOCTOR_NO_CALIBRATION = saved.off;
  }
});
...
test("hook: a Claude session past the threshold is not skipped by the byte fast path", async () => {
  ...
  const out = await new Promise<string>((resolve, reject) => {
    const child = execFile(process.execPath, [cliPath, "hook"], { env: { ...process.env, CONTEXT_DOCTOR_HOOK_STATE: join(dir, "state.json") } },
...
test("proxy: the missing-cache_control advice fires from 1,024 Claude tokens, not 4,000 chars", async () => {
  ...
  const proxy = startProxy({ port: 0, anthropicUpstream: `http://127.0.0.1:${(upstream.address() as AddressInfo).port}` });
...
test("session --list honours --limit", async () => {
  ...
  for (let i = 0; i < 25; i++) writeFileSync(join(proj, `s${i}.jsonl`), JSON.stringify({ type: "user", message: { role: "user", content: "hi" } }));
  const run = (args: string[]) => new Promise<string>((resolve, reject) => {
    execFile(process.execPath, [cliPath, "session", "--list", ...args], { env: { ...process.env, HOME: home, USERPROFILE: home } },
      (err, stdout) => (err ? reject(err) : resolve(stdout)));
  });
