...
export function autopilotPaths(home = homedir()): AutopilotPaths {
...
interface AutopilotConfig {
  port: number;
  /** ANTHROPIC_BASE_URL in settings.json before we set it, restored by `off`. */
...
export function serviceCommand(node: string, cli: string, port: number, paths: AutopilotPaths): string[] {
  return [node, cli, "proxy", "--autopilot", "--port", String(port), "--autopilot-state", paths.statePath, "--autopilot-pause-file", paths.pauseFile];
}

export function launchdPlist(args: string[], log: string): string {
...
export function systemdUnit(args: string[], log: string): string {
...
export function windowsTaskCommand(args: string[]): string {
...
function installService(args: string[], log: string): string {
  const os = platform();
...
function removeService(): void {
  ...
  if (os === "darwin") {
    tryRun("launchctl", ["bootout", `gui/${uid()}`, sp.plist]);
    if (existsSync(sp.plist)) rmSync(sp.plist);
  } else if (os === "linux") {
    tryRun("systemctl", ["--user", "disable", "--now", "context-doctor-proxy.service"]);
    if (existsSync(sp.unit)) rmSync(sp.unit);
...
export function startDetached(args: string[], log: string): void {
...
export async function health(port: number, timeoutMs = 800): Promise<Health> {
  try {
    const r = await fetch(`${proxyUrl(port)}/health`, { signal: AbortSignal.timeout(timeoutMs) });
    const j = (await r.json()) as { ok?: boolean; service?: string; autopilot?: boolean; version?: string };
...
async function waitHealthy(port: number, ms: number): Promise<Health> {
  const end = Date.now() + ms;
  for (;;) {
    const h = await health(port);
    if (h.ok && h.autopilot) return h;
    if (Date.now() > end) return h;
...
export async function autopilotOn(port = DEFAULT_AUTOPILOT_PORT, paths = autopilotPaths()): Promise<{ ok: boolean; lines: string[] }> {
  const lines: string[] = [];
  ...
  writeFileSync(paths.config, JSON.stringify({ port, previousBaseUrl, node: process.execPath, cli } satisfies AutopilotConfig, null, 2));
  if (existsSync(paths.pauseFile)) rmSync(paths.pauseFile);
...
export async function autopilotOff(paths = autopilotPaths()): Promise<string[]> {
  const lines: string[] = [];
  const cfg = readConfig(paths);
  const settings = readJson(paths.settings);
  const env = settings.env as Record<string, string> | undefined;
  if (env?.ANTHROPIC_BASE_URL && cfg && env.ANTHROPIC_BASE_URL === proxyUrl(cfg.port)) {
    if (cfg.previousBaseUrl) env.ANTHROPIC_BASE_URL = cfg.previousBaseUrl;
  ...
  removeService();
  if (existsSync(paths.config)) rmSync(paths.config);
  if (existsSync(paths.pauseFile)) rmSync(paths.pauseFile);
...
export function autopilotPause(paused: boolean, paths = autopilotPaths()): string {
  if (paused) {
...
export async function autopilotStatus(paths = autopilotPaths()): Promise<string[]> {
  const cfg = readConfig(paths);
  if (!cfg) return ["Autopilot is off. `context-doctor autopilot on` turns it on for every new Claude Code session."];
  const lines: string[] = [];
  const h = await health(cfg.port);
  const env = (readJson(paths.settings).env ?? {}) as Record<string, string>;
  ...
  if (existsSync(paths.pauseFile)) lines.push("! Paused: requests pass through unchanged");
  if (h.ok) {
    try {
      const s = (await fetch(`${proxyUrl(cfg.port)}/stats`, { signal: AbortSignal.timeout(1000) }).then((r) => r.json())) as {
        startedAt: string;
        upstreamInputTokens: number; upstreamCacheReadTokens: number; upstreamCacheWriteTokens: number;
        autopilot?: { requests: number; changedRequests: number; batches: number; coldBatches: number; resultsCleared: number; tokensRemoved: number; lastReason: string };
      };
      const a = s.autopilot;
      if (a) {
        lines.push("");
        ...
        if (billed > 0) lines.push(`  Billed input: ${formatTokens(billed)} (${Math.round((s.upstreamCacheReadTokens / billed) * 100)}% cache reads)`);
        if (a.lastReason) lines.push(`  Last decision: ${a.lastReason}`);
      }
    } catch {
 ...
 */
export async function ensureProxyUp(paths = autopilotPaths()): Promise<void> {
  const cfg = readConfig(paths);
  if (!cfg) return;
