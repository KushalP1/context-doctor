...
export function runWatch(opts: WatchOptions): void {
  ...
  const tick = (): void => {
    try {
      ...
      if (size === lastSize) return; // nothing new — cost of this tick was one stat
      lastSize = size;
      ...
      console.log(
        `[${new Date().toISOString().slice(11, 19)}] ~${formatTokens(profile.totalTokens)} tokens` +
