 ...
 */
export function stripToken(url: string, token: string): string | undefined {
  const prefix = "/t/";
...
export function startProxy(opts: ProxyOptions = {}): http.Server {
  ...
  const messageFingerprints = new Map<string, number[]>();
          ...
          try {
            ...
            if (url.startsWith("/v1/messages") && requestModel) {
              ...
              if (stablePrefix.length > 4000 && !hasBreakpoint) {
                ...
                const where = Array.isArray(parsedBody.tools) && (parsedBody.tools as unknown[]).length > 0
                  ? `the last entry in "tools" (tools come before system in the cached prefix)`
                  : `the last block of "system"`;
                advise(
                  `~${Math.round(stablePrefix.length / 4)}+ tokens of stable system/tools on ${requestModel} without cache_control. ` +
              ...
              const msgs = Array.isArray((parsedBody as { messages?: unknown }).messages)
              ...
              if (prevHashes && !hasBreakpoint) {
                let stable = 0;
                while (stable < hashes.length && stable < prevHashes.length && hashes[stable] === prevHashes[stable]) stable++;
                if (stable >= 2) {
                  const stableChars = msgs.slice(0, stable).reduce((n: number, m) => n + JSON.stringify(m).length, 0);
                  ...
                  if (stableTokens >= 1024) {
                    advise(
                      `messages #0-#${stable - 1} (~${stableTokens} tokens) were identical to the previous ${requestModel} request and carry no cache_control. ` +
