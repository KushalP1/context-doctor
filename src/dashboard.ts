...
export interface DashboardData {
  ...
  budget: { path: string; overBudget: boolean; breaches: string[] } | null;
}

async function fetchProxyStats(port: number): Promise<DashboardData["proxy"]> {
  try {
    const res = await fetch(`http://127.0.0.1:${port}/stats`, { signal: AbortSignal.timeout(400) });
    if (!res.ok) return null;
    return (await res.json()) as DashboardData["proxy"];
  } catch {
    return null;
  }
}
...
export async function collectDashboardData(proxyPort = 8787): Promise<DashboardData> {
  ...
  for (const s of listSessions(8)) {
    if (s.sizeBytes > MAX_SESSION_BYTES) continue;
    try {
...
export function startDashboard(opts: { port?: number; proxyPort?: number } = {}): http.Server {
