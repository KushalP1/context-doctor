...
function writtenPath(block: any): string | null {
  if (block?.name === "Write" && typeof block.input?.file_path === "string") return block.input.file_path;
 ...
 */
function editedLaterWithoutRead(messages: any[], writeIndex: number, path: string | null): boolean {
  ...
  for (let j = writeIndex + 1; j < messages.length; j++) {
    ...
    for (const b of content as any[]) {
      ...
      if (b.name === "Read" || /\b(cat|head|tail|sed|less)\b/.test(cmd)) return false; // it re-read: safe
      if (b.name === "Edit" || b.name === "MultiEdit") return true; // edited blind: keep the Write
    }
  }
  return false;
}
 ...
 */
function stableCutoff(messageCount: number, keepRecent: number, step: number = TRIM_BOUNDARY_STEP): number {
 ...
 */
function dropOrphanedToolResults(kept: any[]): void {
  const availableCalls = new Set<string>();
  for (const m of kept) {
    if (!Array.isArray(m?.content)) continue;
    for (const b of m.content) if (b?.type === "tool_use" && b.id) availableCalls.add(b.id);
  }
  for (const m of kept) {
    if (!Array.isArray(m?.content)) continue;
    const surviving = m.content.filter(
      (b: any) => b?.type !== "tool_result" || (b.tool_use_id && availableCalls.has(b.tool_use_id))
    );
    if (surviving.length === m.content.length) continue;
...
export function optimizeConversation(input: string, options: OptimizeOptions = {}): OptimizeResult {
  ...
  const opts: Required<OptimizeOptions> = {
    ...
    trimBoundaryStep:
      options.trimBoundaryStep && options.trimBoundaryStep > 0 ? Math.floor(options.trimBoundaryStep) : DEFAULTS.trimBoundaryStep,
  };
  let data: any;
  try {
    ...
    messages.forEach((m, i) => {
      ...
      if (Array.isArray(m.content)) {
        for (const b of m.content as any[]) {
          if (b?.type !== "tool_use" || b.input == null || typeof b.input !== "object") continue;
