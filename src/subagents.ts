...
function summarize(path: string): SubagentSummary {
  ...
  forEachLine(path, (line) => {
    ...
    if (!line.includes('"usage"') && !line.includes('"timestamp"') && !s.task) return;
    ...
    if (e.type === "user" && !s.task) {
      const c = m.content;
      const text = typeof c === "string" ? c : Array.isArray(c) ? c.map((b: any) => (typeof b === "string" ? b : b?.text ?? "")).join(" ") : "";
  ...
  if (first !== undefined && last !== undefined) s.durationMs = last - first;
  if (priced && s.calls > 0) s.estCostUsd = cost;
  return s;
}

/** Account for every subagent of a session, or null when there are none. */
export function subagentReport(transcriptPath: string): SubagentReport | null {
 ...
 */
export function renderSubagents(report: SubagentReport | null, parentInputUsd?: number, top = 5): string | null {
  ...
  lines.push(
    `${report.agents.length} subagent(s) made ${report.agents.reduce((t, a) => t + a.calls, 0)} API calls: ` +
  ...
  for (const a of report.agents.slice(0, top)) {
    ...
    lines.push(
      `  ${cost.padStart(8)}  ${String(a.calls).padStart(3)} calls  ctx ${formatTokens(a.finalContextTokens).padStart(6)}  ${fmtDuration(a.durationMs).padStart(4)}  ${a.task.slice(0, 60) || "(no task text)"}`
    );
  }
  if (report.agents.length > top) lines.push(`  … and ${report.agents.length - top} more`);
  const heavy = report.agents.filter((a) => a.finalContextTokens > 200_000);
  if (heavy.length > 0) {
    lines.push(
      `${heavy.length} subagent(s) ended above 200k tokens of context. A subagent that big is doing a main session's job; ` +
