...
test("malformed conversations degrade to a warning, never a stack trace", async () => {
    ...
    for (const [name, value] of Object.entries(cases)) {
        ...
        if (Array.isArray(value.messages)) {
            assert.doesNotThrow(() => optimizeConversation(json, { strategies: ["dedupe", "trim-tool-results", "trim-tool-calls", "strip-base64"] }), `optimize: ${name}`);
        }
    }
...
test("dropping malformed entries does not break prune-history", async () => {
    ...
    const messages = [null];
    for (let i = 0; i < 30; i++)
        messages.push({ role: i % 2 ? "assistant" : "user", content: `turn ${i} ${"x".repeat(400)}` });
...
test("files re-read through the shell are detected, command words are not", async () => {
    ...
    for (let n = 0; n < 5; n++)
        messages.push(bash("head -20 /var/log/app.log; echo done", i++));
...
test("usage numbers are coerced, not concatenated", async () => {
    ...
    assert.equal(parseSessionFile(write("strings", { input_tokens: "1200", cache_read_input_tokens: "300" })).reportedInputTokens, 1500, "string usage values must be added, not concatenated");
