...
test("the proxy survives a client that aborts mid-stream", async () => {
    ...
    await new Promise((resolve, reject) => {
        const req = http.request({ host: "127.0.0.1", port: proxyPort, path: "/v1/messages", method: "POST", headers: { "content-type": "application/json" } }, (res) => {
    ...
    const health = await fetch(`http://127.0.0.1:${proxyPort}/health`).then((r) => r.json());
