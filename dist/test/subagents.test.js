...
function line(type, content, extra = {}, ts = "2026-09-18T10:00:00.000Z") {
    return JSON.stringify({ type, timestamp: ts, isSidechain: true, message: { role: type, content, ...extra } });
}
function usage(input, read, write, out, model = "claude-sonnet-5") {
...
test("totals, cache-aware cost, task text, duration and ordering come from the subagent files", () => {
    ...
    writeFileSync(join(subs, "agent-aaa.jsonl"), [
    ...
    writeFileSync(join(subs, "agent-bbb.jsonl"), [line("user", [{ type: "text", text: "summarize the diff" }]), line("assistant", "x", usage(3_000, 0, 0, 100, "totally-unknown-model"))].join("\n") + "\n");
...
test("rendering names the invisible money and compares total against total", () => {
    const { transcript, subs } = session();
    writeFileSync(join(subs, "agent-big.jsonl"), [line("user", "do the whole audit"), line("assistant", "x", usage(1_000, 249_000, 0, 2_000))].join("\n") + "\n");
    const report = subagentReport(transcript);
