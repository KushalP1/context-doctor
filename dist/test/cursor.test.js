...
test("one malformed row does not hide every Cursor chat", { skip }, async () => {
    ...
    for (let i = 0; i < 5; i++) {
        insert.run(`composerData:good${i}`, JSON.stringify({ name: `Chat ${i}`, fullConversationHeadersOnly: [{ bubbleId: "b1", type: 1 }, { bubbleId: "b2", type: 2 }] }));
    }
